...
void ADC_SetInternalReference(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_ADC_ALL_PERIPH(ADCx));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  ...
  if (NewState != DISABLE)
  {
    if(ADCx == ADC1)
	{
		SYSCFG->SYSCFG_CR3 = ((SYSCFG->SYSCFG_CR3 | BIT(15)) | 0xA55A0000);
	}
	else if(ADCx == ADC2)
	{
		SYSCFG->SYSCFG_CR3 = ((SYSCFG->SYSCFG_CR3 | BIT(14)) | 0xA55A0000);
	}
	else if(ADCx == ADC3)
	{
		SYSCFG->SYSCFG_CR3 = ((SYSCFG->SYSCFG_CR3 | BIT(13)) | 0xA55A0000);
	}
  }
  else
  {
