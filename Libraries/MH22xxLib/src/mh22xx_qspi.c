...
void QSPI_DeInit(void)
{
  /* Enable QSPI reset state */
	RCC_AHBPeriphResetCmd(RCC_AHBPeriphReset_QSPI,ENABLE);
	RCC_AHBPeriphResetCmd(RCC_AHBPeriphReset_QSPI,DISABLE);
}
...
void QSPI_ComConfig_Init(QSPI_ComConfig_InitTypeDef* QSPI_ComConfig_InitStruct)
{
  ...
  tmpreg &= 0x6FFFFFFF;//YS 23.3.4, 原库函数没有清DDR位
...
/**
  ...
  * @retval None
  */
void QSPI_WriteComConfig_Init(QSPI_ComConfig_InitTypeDef* QSPI_ComConfig_InitStruct)
{
  uint32_t tmpreg = 0;
  ...
  /*------------------------ QUADSPI CCR Configuration ------------------------*/
  /* Get the QUADSPI CCR value */
  tmpreg = QUADSPI->WR_CCR;
  /* Clear FMODE Mode bits */
  tmpreg &= QSPI_CCR_CLEAR_MASK;
  tmpreg &= 0x6FFFFFFF;//YS 23.3.4, 原库函数没有清DDR位
  /* Configure QUADSPI: CCR Configuration */
  tmpreg |=  (uint32_t)( (QSPI_ComConfig_FMode_Indirect_Write)
                       | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DDRMode)
					   | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DHHC)
                       | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_SIOOMode)
                       | (QSPI_ComConfig_InitStruct->QSPI_ComConfig_DMode)
...
/**
  ...
  *          This parameter can be any value between 0x00000000 and 0xFFFFFFFF.
  * @param  QSPI_Match_Mode: indicates which method should be used for determining a 搈atch?during
...
/**
  * @brief  Enables or disables QSPI Memory Map Write Mode.
  * @param  NewState: new state of the QSPI Memory Map Write Mode. 
  *          This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
void QSPI_MMW_Cmd(FunctionalState NewState)
{
	if (NewState != DISABLE)
	{
	/* Enable QSPI peripheral */
		QUADSPI->CR1 |= QUADSPI_CR1_REMAP_WR_EN;
	}
	else
	{
	/* Disable QSPI peripheral */
...
/**
  * @brief  Enables or disables QSPI Crypt Mode.
  * @param  NewState: new state of the QSPI Crypt Mode. 
  *          This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
void QSPI_CRYPT_Cmd(FunctionalState NewState)
{
	if (NewState != DISABLE)
	{
	/* Enable QSPI peripheral */
		QUADSPI->CR1 |= QUADSPI_CR1_CRYPT_EN;
	}
	else
	{
	/* Disable QSPI peripheral */
