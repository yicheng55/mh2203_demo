...
本文件說明 `ModuleDemo/DM9051A/dm9051_driver` 目錄下的 DM9051A SPI 乙太網路驅動程式結構。此驅動以 mh2030 / MH2203 MCU 為主要平台，並以分層方式整理 DM9051A 晶片核心邏輯、硬體抽象層、平台移植層與 TCP/IP stack 適配層。
...
| `ports/mh2203/dm9051_hal_mh2203_spi1.c` | 將 MH2203 的 SPI1 polling 傳輸、CS 控制、reset GPIO、delay 綁定到 DM9051 HAL。 | MH2203 平台移植層 |
| `ports/mh2203/dm9051_hal_mh2203_spi1.h` | 宣告 MH2203 SPI1 HAL binding API 與設定結構。 | 平台移植層對外介面 |
| `ports/mh2203/dm9051_hal_mh2203_spi1_priv.h` | 放置 SPI1 與 pin mapping 的私有共用定義。 | 平台私有定義 |
| `ports/mh2203/dm9051_hal_mh2203_spi1_dma.c` | 提供 SPI1 DMA 版本 FIFO 傳輸實作。 | 平台 DMA 傳輸層 |
| `ports/mh2203/dm9051_hal_mh2203_spi1_dma.h` | 宣告 SPI1 DMA 相關介面。 | 平台 DMA 介面 |
| `ports/mh2203/dm9051_hal_mh2203_int.c` | 實作 DM9051 INT 腳位對應的 EXTI/NVIC 中斷設定與 handler 轉接。 | 平台中斷處理層 |
| `ports/mh2203/dm9051_hal_mh2203_int.h` | 宣告 MH2203 中斷初始化與 handler API。 | 平台中斷介面 |
| `ports/mh2203/mh2203_platform.h` | 集中包含 MH2203 平台相關 header，例如 `mh20xx.h`、delay 等。 | 平台相容封裝層 |
| `ports/mh2203/mh2203_board.c` | 實作 board bring-up，例如 clock、debug UART、printf retarget。 | 板級支援層 |
| `ports/mh2203/mh2203_board.h` | 宣告 board 初始化相關 API。 | 板級支援介面 |
| `ports/mh2203/delay.c` | 提供 MH2203 使用的 delay 實作。 | 時序支援層 |
| `ports/mh2203/delay.h` | 宣告 delay API。 | 時序支援介面 |
| `ports/mh2203/mh2203_uip_clock.c` | 提供 uIP 使用的系統 clock/timer 支援。 | uIP 平台時間層 |
| `ports/mh2203/mh2203_uip_clock.h` | 宣告 uIP clock 相關 API。 | uIP 平台時間介面 |
| `ports/mh2203/mh20xx_it.c` | 放置 MH2203 中斷向量或中斷服務函式整合。 | MCU 中斷整合層 |
...
| `dm9051_env.c` | `hal/inc/dm9051_hal.h`、`ports/mh2203/*` | SPI、GPIO、IRQ、delay 等硬體相關操作改由 HAL contract 與 MH2203 port 實作。 |

## 3. 硬體接線與配置 (Hardware Configuration)

目前 MH2203 port 的 DM9051A 腳位配置如下：
...
MH2203 平台相關 SPI、GPIO、EXTI、delay 實作集中在 `ports/mh2203/`，核心層不直接包含 MCU vendor header。

## 4. 驅動初始化與使用說明 (Getting Started)

### 4.1 HAL 介面重點

`hal/inc/dm9051_hal.h` 是 DM9051A driver 可移植性的核心。它不直接依賴 MH2203，也不包含 SPI/GPIO 的具體實作，而是定義一組平台必須提供的操作函式。

| HAL 成員 | 功能 | 由誰實作 | 說明 |
|---|---|---|---|
| `read_reg` | 讀取 DM9051 暫存器 | `ports/mh2203/*` | 透過 SPI 送出 register read command，讀回單一暫存器值。 |
| `write_reg` | 寫入 DM9051 暫存器 | `ports/mh2203/*` | 透過 SPI 寫入單一 DM9051 暫存器。 |
| `read_mem` | 從 RX FIFO 讀取資料 | `ports/mh2203/*` | 通常對應 DM9051 `MRCMD` 流程，用於接收 Ethernet frame。 |
| `write_mem` | 寫入 TX FIFO | `ports/mh2203/*` | 通常對應 DM9051 `MWCMD` 流程，用於送出 Ethernet frame。 |
| `reset` | 控制硬體 reset | `ports/mh2203/*` | 可透過 GPIO 控制 DM9051 reset pin。 |
| `delay_ms` | 毫秒延遲 | `ports/mh2203/delay.*` | 用於 reset、PHY 操作或初始化等待。 |
| `delay_us` | 微秒延遲 | `ports/mh2203/delay.*` | 用於 SPI/PHY busy wait 等短延遲。 |
| `irq_enable` | 啟用 MCU 端 DM9051 中斷 | `ports/mh2203/dm9051_hal_mh2203_int.*` | 控制 EXTI/NVIC，而不是直接修改 DM9051 IMR。 |
| `irq_disable` | 停用 MCU 端 DM9051 中斷 | `ports/mh2203/dm9051_hal_mh2203_int.*` | 避免中斷重入或進入臨界區時使用。 |
...
| 硬體存取 | `read_reg` / `write_reg` | 透過 MH2203 SPI 實際存取 DM9051。 |
...
| 實際 SPI 傳輸 | `write_mem` / `write_reg` | 由 MH2203 port 完成 SPI 寫入。 |

## 5. 常見問題與排錯 (Troubleshooting)

| 問題現象 | 優先檢查檔案 | 檢查重點 |
|---|---|---|
| 無法讀到 DM9051 chip ID | `ports/mh2203/dm9051_hal_mh2203_spi1.c`、`core/src/dm9051_core.c` | SPI mode、CS 腳位、reset timing、`VIDL/VIDH/PIDL/PIDH` 讀值。 |
...
| 中斷模式無反應 | `ports/mh2203/dm9051_hal_mh2203_int.c` | EXTI line、NVIC enable、INT pin polarity、core interrupt flag。 |
| DMA 模式異常 | `ports/mh2203/dm9051_hal_mh2203_spi1_dma.c` | DMA channel、transfer length、FIFO command sequence、cache/資料對齊問題。 |
...
| 傳輸模式 | 建議位置 | 說明 |
|---|---|---|
| SPI polling | `ports/mh2203/dm9051_hal_mh2203_spi1.c` | 最基本、最容易除錯的傳輸方式。 |
| SPI DMA | `ports/mh2203/dm9051_hal_mh2203_spi1_dma.c` | 適合大量 RX/TX FIFO 傳輸。 |
| SPI interrupt | 可新增 `dm9051_hal_mh2203_spi1_it.c` | 適合非阻塞式傳輸架構。 |
| RTOS SPI wrapper | 可新增 `dm9051_hal_mh2203_spi1_rtos.c` | 適合 FreeRTOS task/queue/semaphore 架構。 |
...
| Port layer 不包含協定棧邏輯 | MH2203 port 只負責 MCU peripheral 與 HAL binding。 |
...
後續維護時，最重要的是守住 layer boundary。只要避免讓 core 直接依賴 MH2203，也避免讓 adapter 直接碰 DM9051 register，這份 driver 之後要支援 DMA、中斷、不同 TCP/IP stack 或不同 MCU，都會比較穩定。
