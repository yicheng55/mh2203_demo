...
uint16_t FLASH_BUF[FLASH_SECTOR_SIZE/2];//最多是2K字节
void FLASH_Write(uint32_t WriteAddr,uint16_t *pBuffer,uint16_t NumToWrite)	
{
	...
	if(WriteAddr<MH2103_FLASH_BASE||(WriteAddr>=(MH2103_FLASH_BASE+128*FLASH_SECTOR_SIZE)))return;//非法地址
	FLASH_Unlock();						//解锁
	offaddr=WriteAddr-MH2103_FLASH_BASE;		//实际偏移地址.
	...
	while(1) 
	{	
		FLASH_Read(secpos*FLASH_SECTOR_SIZE+MH2103_FLASH_BASE,FLASH_BUF,FLASH_SECTOR_SIZE/2);//读出整个扇区的内容
		...
		if(i<secremain)//需要擦除
		{
			FLASH_ErasePage(secpos*FLASH_SECTOR_SIZE+MH2103_FLASH_BASE);//擦除这个扇区
			for(i=0;i<secremain;i++)//复制
			{
				FLASH_BUF[i+secoff]=pBuffer[i];	  
			}
			FLASH_Write_NoCheck(secpos*FLASH_SECTOR_SIZE+MH2103_FLASH_BASE,FLASH_BUF,FLASH_SECTOR_SIZE/2);//写入整个扇区  
