...
BYTE WriteBuffer[] =  "MEGAHUNT兆讯恒达微电子，新建文件系统测试文件\n";       //写缓冲区
...
{	
	...
	PRINTF_LOG("SDIO SD Card Fatfs Test.\n");
	TestList();
...
void TestList(void)
{
	...
	PRINTF_LOG("2: 大数据量读写(FatFs读写测试文件.txt),进行读写测试\n");
	PRINTF_LOG("3: 显示SD Card 根目录下的文件测试\n");
	PRINTF_LOG("4: 创建目录(/Dir1,/Dir1/Die1_1,/Dir2)\n");
	PRINTF_LOG("5: 删除文件和目录(/Dir1,/Dir1/Dir1_1,/Dir2,FatFs读写测试文件.txt)\n");
	PRINTF_LOG("****************************************************************************/\n");
}

void SDInfoShow(void)
{
	PRINTF_LOG("/***************************SD Info Show*******************************/\n");
	PRINTF_LOG("SDCardInfo.CardType : %d\n",SDCardInfo.CardType);
	PRINTF_LOG("SDCardInfo.CardCapacity : %d Byte\n",(SDCardInfo.CardCapacity));
	PRINTF_LOG("SDCardInfo.CardBlockSize : %d Byte\n",SDCardInfo.CardBlockSize);
}
...
//	res_sd = f_unlink("WriteFileTest????.txt");
//	if(res_sd == FR_OK)
//	{
//		DEBUG("??WriteFileTest????.txt??\r\n");
//	}
//	else if((res_sd == FR_NO_FILE)||(res_sd == FR_NO_PATH))
//	{
//		DEBUG("????????? : %s\r\n","/WriteFileTest????.txt");
