...
void QSPI_M2M_DMA_Config(uint32_t pdatasize, uint32_t mdatasize)
{
	DMA_InitTypeDef DMA_InitStructure;
	...
	DMA_InitStructure.DMA_PeripheralDataSize	= pdatasize;
	DMA_InitStructure.DMA_MemoryDataSize		= mdatasize;
...
void QSPI_Configuration(void)
{
	...
	QSPI_GPIO_Config();
	QSPI_DMA_Config();
	QSPI_M2M_DMA_Config(DMA_PeripheralDataSize_Byte,DMA_MemoryDataSize_Byte);
...
void QSPI_M2M_DMA(uint32_t Addr,uint8_t *buf,uint16_t len)
{
...
{
	{
		...
		memset(Test_Recv_Buf,0x00,sizeof(Test_Recv_Buf));
		
		QSPI_M2M_DMA((uint32_t)Test_Send_Buf,(uint8_t *)0xB1000000,sizeof(Test_Send_Buf));
		while(DMA_GetFlagStatus(QSPI_M2M_DMA_FLAG_TC) == RESET);
		DMA_ClearFlag(QSPI_M2M_DMA_FLAG_TC);
		DMA_Cmd(QSPI_M2M_DMA_CHANNEL,DISABLE);
		
		PRINTF_LOG("DMA Write(Byte) 16K Byte OK\n");
	...
	
	{
		...
		QSPI_M2M_DMA_Config(DMA_PeripheralDataSize_HalfWord,DMA_MemoryDataSize_HalfWord);
		for(uint16_t i = 0; i < sizeof(Test_Send_Buf); i++)
			Test_Send_Buf[i] = i;
		
		memset(Test_Recv_Buf,0x00,sizeof(Test_Recv_Buf));
		QSPI_M2M_DMA((uint32_t)Test_Send_Buf,(uint8_t *)0xB1000000,sizeof(Test_Send_Buf)/2);
		while(DMA_GetFlagStatus(QSPI_M2M_DMA_FLAG_TC) == RESET);
		DMA_ClearFlag(QSPI_M2M_DMA_FLAG_TC);
		DMA_Cmd(QSPI_M2M_DMA_CHANNEL,DISABLE);
		
		PRINTF_LOG("DMA Write(Halfword) 16K Byte OK\n");
	...
	
	{
		...
		QSPI_M2M_DMA_Config(DMA_PeripheralDataSize_Word,DMA_MemoryDataSize_Word);
		for(uint16_t i = 0; i < sizeof(Test_Send_Buf); i++)
			Test_Send_Buf[i] = i;
		
		memset(Test_Recv_Buf,0x00,sizeof(Test_Recv_Buf));
		QSPI_M2M_DMA((uint32_t)Test_Send_Buf,(uint8_t *)0xB1000000,sizeof(Test_Send_Buf)/4);
		while(DMA_GetFlagStatus(QSPI_M2M_DMA_FLAG_TC) == RESET);
		DMA_ClearFlag(QSPI_M2M_DMA_FLAG_TC);
		DMA_Cmd(QSPI_M2M_DMA_CHANNEL,DISABLE);
		
		PRINTF_LOG("DMA Write(Word) 16K Byte OK\n");
