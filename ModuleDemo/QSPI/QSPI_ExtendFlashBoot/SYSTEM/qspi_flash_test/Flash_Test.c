...
void Flash_Init(void)
{
	...
	#else
	memcpy(&Temp_Flash_Cmd,&Flash_Cmd[FLASH_CMD_QUAD_INOUT_FAST_READ_DTR],sizeof(Flash_CMD_t));
	#endif
	Temp_Flash_Cmd.Functional_Mode = QSPI_ComConfig_FMode_Memory_Mapped;
	#if 0
	QSPI_ComConfig_InitStructure.QSPI_ComConfig_SIOOMode = QSPI_ComConfig_SIOOMode_Enable;
	QSPI_SetAlternateByte(0x20);
	#else
	QSPI_SetAlternateByte(0x00);
	#endif
...
void QSPI_Configuration(void)
{
	...
	QSPI_InitStructure.QSPI_Prescaler = 1;			           //QSPI 预分频器值
