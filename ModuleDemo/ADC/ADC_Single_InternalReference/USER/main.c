...
#define ADC_TEST_CHANNEL_PIN	(GPIO_Pin_4)
uint8_t ADC_CovChannel[1] = {ADC_Channel_4};
...
{	 
	...
	PRINTF_LOG("ADC Single Internal Reference Test\n");
	PRINTF_LOG("ADC1 内部参考基准Code: %d\n",*(uint32_t *)(0xE0042040));
	PRINTF_LOG("ADC2 内部参考基准Code: %d\n",*(uint32_t *)(0xE0042044));
	PRINTF_LOG("ADC3 内部参考基准Code: %d\n",*(uint32_t *)(0xE0042048));
	
	ADC_VREF[0]= 3300 * (*(uint32_t *)(0xE0042040))/4095;
	ADC_VREF[1]= 3300 * (*(uint32_t *)(0xE0042044))/4095;
	ADC_VREF[2]= 3300 * (*(uint32_t *)(0xE0042048))/4095;
...
{ 	
	...
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_SYSCFG, ENABLE );
	...
	ADC_SetInternalReference(ADC1, ENABLE);
...
void DMA1_Channel1_IRQHandler(void)
{
	if(DMA_GetITStatus(DMA1_IT_TC1) != RESET)
	{
		...
		(float)ADC_VREF[0] * DAM_ADC_Value[0]/4095/1000); 
	}
}
