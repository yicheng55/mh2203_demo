...
uint32_t Write_AT_Data(uint32_t u32Addr, uint32_t u32Data)
{
		FLASHStatus = FLASH_ProgramWord(u32Addr, u32Data);
	#ifdef TL_FLASH_NOT_WRITE_DBG
//		display_flash_data(u32Addr, TEST_PATTERN); //printf(".HINT ..
		u32Addr_d[(u32Addr- StartAddress) >> 2] = u32Data;
	#endif
		return u32Addr + 4;
}
...
void Write_AT_Show_DataFlash(void)
{
	  ...
	  if(FLASHStatus == FLASH_PRC_DONE){
			...
			u32Addr = Write_AT_Data(StartAddress, TEST_PATTERN);
...
uint32_t Read_AT_Data(uint32_t *p32Addr)
{
	uint32_t u32Data = *((volatile uint32_t *)*p32Addr);
#ifdef TL_FLASH_NOT_WRITE_DBG
	u32Addr_d[(*p32Addr- StartAddress) >> 2] = u32Data;
#endif
	*p32Addr += 4;
	return u32Data;
}
