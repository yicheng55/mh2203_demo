...
#if 1
		//if (1) { } essential set destination ip, for _arp_need to check. Jos.
		do {
		  uip_ipaddr_t ipaddr_data;
		  uip_ipaddr(ipaddr_data, at_type.udp_raddr[0], at_type.udp_raddr[0] >> 8, at_type.udp_raddr[1], at_type.udp_raddr[1] >> 8);
...
#if 0	  
		  printf("NEED arp FOUND\r\n");
//		  printf("%d tabptr->ethaddr.addr ", UIP_ARPTAB_SIZE);
//		  for (i=0; i<6; i++)
//			 printf("%02x ", tabptr->ethaddr.addr[i]);
...
extern volatile uint8_t StringLoop;

void USART2_IRQ_Routine1(void)
{
    /* MH2203 ATCMD Web uses USART2 RXNE in at_port.c, not AT32 DMA RX. */
}
//#endif //UPDATE_USART2_RX_DMA
#endif //!defined(ATCMD_UART_RX_DOUB_BUF) && !defined(UIP_USE_BIGDATA_SIZE) && !defined(UIP_USE_BIGDATA_SIZE)

#ifndef ATCMD_UART_RX_DOUB_BUF	
void RXDMA_IRQ_Routine1(void)
{
    /* MH2203 ATCMD Web does not use AT32 DMA RX. */
}
#endif //#ndef ATCMD_UART_RX_DOUB_BUF
