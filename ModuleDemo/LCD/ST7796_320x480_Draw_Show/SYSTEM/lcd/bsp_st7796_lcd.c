...
__inline void                 ST7796_Write_Cmd           ( uint8_t usCmd );
__inline void                 ST7796_Write_Data          ( uint8_t usData );
...
static __inline void          ST7796_FillColor           ( uint32_t ulAmout_Point, uint16_t usColor );
...
void LCD_Clear(u16 color)
{
  ...
  LCD_Set_Window(0,0,320,480);
	ST7796_Write_Cmd(0x2c);     //开始写入GRAM	
	for(index=0; index<154560; index++)
	{
...
{	
	if ( ( usX < LCD_X_LENGTH ) && ( usY < LCD_Y_LENGTH ) )
  {
	...
	#if DMA_TEST
	ST7796_Write_Cmd ( CMD_SetCoordinateX ); 				 /* 设置X坐标 */
	Send_Data	[0]=((usX>>8)&0xff)|((usX&0xff)<<8);
	Send_Data	[1]=((usX>>8)&0xff)|((usX&0xff)<<8);
	Dma_Send_Data(Send_Data,4);
	ST7796_Write_Cmd ( CMD_SetCoordinateY ); 			     /* 设置Y坐标*/
	Send_Data	[0]=((usY>>8)&0xff)|((usY&0xff)<<8);
	Send_Data	[1]=((usY>>8)&0xff)|((usY&0xff)<<8);
	Dma_Send_Data(Send_Data,4);
	ST7796_Write_Cmd (CMD_SetPixel);	
	Send_Data	[0]=((CurrentTextColor>>8)&0xff)|((CurrentTextColor&0xff)<<8);
	Dma_Send_Data(Send_Data,2);
#endif
...
void ST7796_DispChar_EN ( uint16_t usX, uint16_t usY, const char cChar )
{
	...
	for ( byteCount = 0; byteCount < fontLength; byteCount++ )
	{
		...
		for ( bitCount = 0; bitCount < 8; bitCount++ )
		{
				...
				if ( Pfont[byteCount] & (0x80>>bitCount) )
				{
					Send_Data	[i]=((CurrentTextColor>>8)&0xff)|((CurrentTextColor&0xff)<<8);
					i++;
				}						
				else
				{
					Send_Data	[i]=((CurrentBackColor>>8)&0xff)|((CurrentBackColor&0xff)<<8);
					i++;
				}
		}
	}
	Dma_Send_Data(Send_Data,fontLength*8*2);
	#endif
}
void Dma_Send_Data(uint16_t * data,uint32_t Data_Len)
{
	DMA_Cmd(DMA1_Channel1, DISABLE);
	DMA1_Channel1->CMAR=(uint32_t)data;
	DMA1_Channel1->CPAR=(uint32_t)(FSMC_Addr_ST7796_DATA);
...
void Dma_Init(void)
{
	  ...
	  DMA_Cmd(DMA1_Channel1, DISABLE);
		...
		DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(FSMC_Addr_ST7796_DATA);//外设站点的基地址
