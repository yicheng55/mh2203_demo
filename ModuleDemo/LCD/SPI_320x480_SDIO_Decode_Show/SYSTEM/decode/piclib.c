...
//	LCD_Fill(x0,y0,x0+len-1,y0,color);	
	...
	ST7796_OpenWindow (x0,y0,(len),1);
	ST7796_FillColor( (len) * 1 ,color);	
}
//填充颜色
//x,y:起始坐标
//width，height：宽度和高度。
...
//	LCD_Color_Fill(x,y,x+width-1,y+height-1,color);	
	ST7796_OpenWindow (x,y,width,height);
	ST7796_FillColor( (width-x) * (height-y) ,*color);	
}
//////////////////////////////////////////////////////////////////////////
...
{
	pic_phy.read_point=ST7796_ReadPoint;  		//读点函数实现,仅BMP需要
	pic_phy.draw_point=ST7796_Fast_DrawPoint;	//画点函数实现
	pic_phy.fill=ST7796_Fill;					//填充函数实现,仅GIF需要
