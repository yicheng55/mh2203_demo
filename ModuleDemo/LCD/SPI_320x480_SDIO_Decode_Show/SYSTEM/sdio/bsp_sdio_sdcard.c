...
void SD_DeInit(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_11 | GPIO_Pin_0 | GPIO_Pin_1;
...
static void GPIO_Configuration(void)
{
	...
	GPIO_PinRemapConfig(GPIO_Remap_SDIO_AF1,ENABLE);
...
SD_Error SD_Init(void)
{
	...
	*/
	...
	if (errorstatus == SD_OK)
	{
		errorstatus = BusWidth_Config(SDIO_BusWide_1b);
	}  
...
//			return(errorstatus);
			...
			SDIO_InitStructure.SDIO_ClockDiv = SDIO_TRANSFER_CLK_DIV + 127; 
...
//						Delay_Ms(100);
					...
					//	SDIO_SendCommand(&SDIO_CmdInitStructure);//WAITPEND测试
						
					//	SDIO->DCTRL |= 1;//发送过程中写DTEN=1
					}
					tempbuff += 8;
					bytestransferred += 32;
				}
...
{ 
	...
	fifoCnt[0]=SDIO_GetFIFOCount();//zzz
...
/*********以下5行测试FIFOCNT  zzz**********/
	...
	{
		SDIO_ReadData();
		fifoCnt[i]=SDIO_GetFIFOCount();
	}	
	if((fifoCnt[0]==2)&&(fifoCnt[1]==0)&&(fifoCnt[2]==0)&&(fifoCnt[3]==0)) TESTDEBUG("read FIFOCNT：PASS\n");
	else TESTDEBUG("read FIFOCNT：FAIL,data0=%x,data1=%x,data2=%x,data3=%x\n",fifoCnt[0],fifoCnt[1],fifoCnt[2],fifoCnt[3]);
...
SD_Error SD_WriteBlockWaitPend(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize,uint32_t DataMode)
{
	SD_Error errorstatus = SD_OK;
