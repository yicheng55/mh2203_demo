...
#define SCR_WD  320//128
#define SCR_HT  480//160

#define WD_3D   320//128
#define HT_3D   (480-16)//320//128
...
#define NUM_STARS 150 //星星数量
Star stars[NUM_STARS];
int starSpeed = 20;//星星速度
...
int t=0;
extern void ST7796_Write_Cmd ( uint8_t usCmd );
extern void ST7796_Write_Data ( uint8_t usData );
void LCD_Draw3DBuf(uint16_t _usX, uint16_t _usY, uint16_t _usHeight, uint16_t _usWidth, uint16_t *_ptr)
{
	uint32_t i;
	uint16_t y;
	uint16_t *p;
	...
	ST7796_OpenWindow(0,y+16,WD_3D,NLINES);
//	printf("%d\n",y+NLINES);
	...
	#if 1//CPU 执行
	for ( i = 0; i < WD_3D*NLINES; i ++ )
	{
		ST7796_Write_Data ( *(uint16_t *)p >> 8 & 0xFF);
		ST7796_Write_Data ( *(uint16_t *)p & 0xFF);
		p++;
	}
	#endif
	...
	#if 0//DMA 执行
	{
		...
		DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)p;//存储器站点的基地址
		DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
		DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
		DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;//传输方向
		DMA_InitStructure.DMA_BufferSize = (WD_3D*NLINES*2);//传输次数
...
void render3D(void)
{
  ...
  if(t++>360) t-=360;
  distToObj = 150 + 300*fastSin(3*t)/MAXSIN;//沿z轴移动幅度，调整数字300大小
  cos0 = fastCos(rot0);
  sin0 = fastSin(rot0);
  cos1 = fastCos(rot1);
  sin1 = fastSin(rot1);
  ...
  for(i=0;i<HT_3D;i+=NLINES) {
    ...
    LCD_Draw3DBuf(0, yFr, NLINES, SCR_WD, frBuf);
  }
...
// --------------------------------------------------------------------------
unsigned int ms,msMin=1000,start=0,msMax=0;
char txt[30];
uint32_t frames=0;
